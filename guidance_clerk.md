...
```bash
# .env.local
NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY=pk_test_ZmluZXItZWxrLTE3LmNsZXJrLmFjY291bnRzLmRldiQ
CLERK_SECRET_KEY=sk_test_DJ9mpnwRnN7vSegLu7fikMDGIG4Ws0mK9pi7KYgoHa
```
...
## **7. STONESOUP PROJECT SPECIFIC NOTES**

### Environment Variables Setup
Your Clerk keys have been added to `.env`:

```bash
# Backend (Python/FastAPI)
CLERK_SECRET_KEY=sk_test_DJ9mpnwRnN7vSegLu7fikMDGIG4Ws0mK9pi7KYgoHa
CLERK_PUBLISHABLE_KEY=pk_test_ZmluZXItZWxrLTE3LmNsZXJrLmFjY291bnRzLmRldiQ
...
2. Create or update `middleware.ts` with `clerkMiddleware()`
3. Wrap your app with `<ClerkProvider>` in `app/layout.tsx`
4. Add authentication UI components where needed
5. Test sign-in/sign-up flow

### Common Beginner Tips
- **Environment Variables**: Make sure to use `NEXT_PUBLIC_` prefix for client-side variables
- **Middleware**: The middleware file goes at the root of your project (same level as `package.json`)
- **Components**: Clerk components handle all the authentication UI for you - no need to build custom forms
- **Testing**: You can create test accounts using the Clerk dashboard during development

### Troubleshooting
- If authentication isn't working, check that your environment variables are correctly set
- Make sure you're using `clerkMiddleware()` and not the deprecated `authMiddleware()`
- Verify that `<ClerkProvider>` wraps your entire app in the layout file
- Check the browser console for any import errors or missing dependencies
